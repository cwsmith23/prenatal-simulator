...
def build_financials(df, p):
    ...
    bs["Total Current Assets"] = bs["Cash Balance"] + bs["Inventory"]
    ...
    bs = bs[[
        "Cash Balance","Inventory","Total Current Assets",
...
# Dynamic borders for subtotals
subtotal_idxs = [
    i + 1
    for i, (_, label) in enumerate(formatted_bs.index)
    if label.lower().startswith("total")
]
border_styles = [
    {"selector": f"tbody tr:nth-child({pos})", "props": [("border-bottom", "2px solid #000")]}
    for pos in subtotal_idxs
]
